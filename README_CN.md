...
Cabinet V2.0 是一个 **TypeScript 单体仓库**，建立在严格的 4 层架构之上。15 个包和 2 个应用按依赖方向组织——下层绝不依赖上层。
...
- **MCP 集成 · 外部工具生态**
  通过 stdio 或 SSE 连接外部 MCP 服务器，动态注册工具、资源和提示模板——无需修改代码即可打开通往外部世界的门。

- **交互式子代理 · 多轮智能体会话**
  生成拥有独立会话状态的交互式子代理，支持中途用户输入、事件驱动状态同步，让复杂任务可中途修正航向。

- **多层记忆 · 你的外挂大脑**
  短期会话上下文 → WriteGate 质量过滤 → CascadeBuffer 暂存 → 长期语义检索（SQLite + HNSW）→ KnowledgeGraph 实体提取 + 矛盾检测 → MemoryDecay 生命周期管理。整合沉淀，项目间隔离。
...
- **可观测性 · 透明可审计**
  内置可观测性采集器追踪会话指标、工具健康度、成本、上下文区域分布和 MCP 服务器状态。生成日报与健康检查，运行时仪表盘支持 WebSocket 实时更新。你的 AI 团队运转从不黑箱。
...
在 `~/.cabinet/mcp/*.json` 或桌面端设置界面中配置 MCP 服务器。支持 stdio（本地进程）和 SSE（远程服务器）两种传输方式：
...
Cabinet V2.0 采用基于 SQLite 的多层记忆架构：

- **ShortTerm**：会话上下文，内存 + SQLite，TTL 30 分钟
- **WriteGate**：5 层正则分类过滤噪音
- **CascadeBuffer**：L0 暂存区，批量压缩后写入长期记忆
- **LongTerm**：跨会话语义检索（SQLite + HNSW 向量索引），RRF 混合搜索
- **KnowledgeGraph**：实体提取 + 矛盾检测
- **MemoryDecay**：基于 Ebbinghaus 遗忘曲线的生命周期管理
...
Cabinet 的设计深受控制论原则启发——递归可行系统、闭环认知和多样性匹配。针对 8 项控制原则的完整架构审计参见 [`AUDIT_REPORT.md`](./AUDIT_REPORT.md)。
