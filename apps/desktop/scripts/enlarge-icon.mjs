...
async function createEnlargedSource() {
  ...
  console.log(
    `Content bounds: x=${bounds.minX}, y=${bounds.minY}, w=${bounds.width}, h=${bounds.height}`,
  );

  const CANVAS = 512;
  // Target 80% fill in the limiting dimension so content is prominent but doesn't overflow
  const SCALE = Math.min((CANVAS * 0.8) / bounds.width, (CANVAS * 0.8) / bounds.height);
...
async function generatePngs() {
  ...
  for (const { file, size } of sizes) {
    ...
    await sharp(SOURCE_PNG).resize(size, size, { kernel: sharp.kernel.lanczos3 }).png().toFile(out);
    console.log(`  ${file}`);
  }
}
...
async function generateIco() {
  ...
  try {
    ...
    toIcoModule = await import(
      pathToFileURL(join(tmpDir, 'node_modules', 'to-ico', 'index.js')).href
    );
  }
  ...
  const buffers = await Promise.all(
    sizes.map((s) =>
      sharp(SOURCE_PNG).resize(s, s, { kernel: sharp.kernel.lanczos3 }).png().toBuffer(),
    ),
  );
...
async function generateIcns() {
  ...
  try {
    execSync(`npx png2icons "${SOURCE_PNG}" "${join(ICONS_DIR, 'icon')}" -icns`, {
      stdio: 'inherit',
    });
