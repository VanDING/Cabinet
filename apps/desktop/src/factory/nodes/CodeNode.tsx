...
export function CodeNode({ data, selected }: NodeProps) {
  ...
  return (
    <>
      ...
      <div className={`rounded-md border px-3 py-2 min-w-[130px] max-w-[200px] shadow-sm transition-shadow wf-border-info-40 wf-bg-info-15 ${selected ? 'shadow-md ring-2 wf-ring-info-50' : ''}`}>
        ...
        {code ? (
          <p className="mt-1 rounded wf-bg-info-25 px-1.5 py-0.5 text-[10px] text-content-tertiary font-mono truncate">{code.slice(0, 50)}{code.length > 50 ? '...' : ''}</p>
        ) : null}
