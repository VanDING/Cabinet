...
export function ParallelNode({ data, selected }: NodeProps) {
  ...
  return (
    <>
      ...
      <div className={`rounded-xl border-2 min-w-[150px] overflow-hidden shadow-sm transition-shadow wf-border-info-30 wf-bg-info-5 ${selected ? 'shadow-md ring-2 wf-ring-info-40' : ''}`}>
        <div className="flex items-center gap-1.5 wf-bg-info-20 px-3 py-1.5">
          <span className="text-sm text-intent-info-foreground">≣</span>
          <span className="text-xs font-semibold text-intent-info-foreground">Parallel</span>
        </div>
        <div className="px-3 py-2">
          <p className="text-xs font-medium text-content-primary">{title}</p>
          <div className="mt-1.5 flex gap-1">
            <div className="h-1.5 flex-1 rounded-sm wf-bg-info-20" />
