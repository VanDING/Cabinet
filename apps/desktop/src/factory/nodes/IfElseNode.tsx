...
export function IfElseNode({ data, selected }: NodeProps) {
  ...
  return (
    <>
      ...
      <div
        className={`relative flex flex-col items-center justify-center min-w-[160px] h-[85px] shadow-sm transition-shadow wf-border-warning-30 wf-bg-warning-5 ${selected ? 'shadow-md' : ''}`}
        style={{
      ...
      >
        <span className="text-sm text-intent-warning-foreground">{'<>'}</span>
        <span className="text-xs font-semibold text-intent-warning-foreground mt-0.5">{title || 'If-Else'}</span>
