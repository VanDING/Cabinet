...
function EntityCard({ data }: { data: Record<string, unknown> }) {
  ...
  const decisions = getArr(data, 'decisions') as unknown[] || getArr(data, 'decisionHistory') as unknown[];
  const total = decisions.length || getNum(data, 'totalDecisions') || 0;
  const approved = decisions.filter((d) => isRecord(d) && d.action === 'approved').length || getNum(data, 'approved') || 0;
  const rejected = decisions.filter((d) => isRecord(d) && d.action === 'rejected').length || getNum(data, 'rejected') || 0;
        ...
        <div className="space-y-1">
          ...
          <div className="max-h-40 overflow-y-auto space-y-1">
            {decisions.slice(0, 20).map((d, i) => {
              ...
              const badgeColor = action === 'approved' ? 'bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-300'
                : action === 'rejected' ? 'bg-red-100 text-red-700 dark:bg-red-900/40 dark:text-red-300'
                : 'bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-400';
              return (
                <div key={i} className="flex items-center justify-between rounded border px-2 py-1 dark:border-gray-700">
                  <span className="truncate text-[11px] text-gray-700 dark:text-gray-300" title={title}>{title}</span>
                  <div className="flex flex-shrink-0 items-center gap-2">
                    {date && <span className="text-[10px] text-gray-400">{new Date(date).toLocaleDateString()}</span>}
                    <span className={`rounded px-1.5 py-0.5 text-[10px] font-medium ${badgeColor}`}>{action}</span>
                  </div>
                </div>
              );
            })}
...
function ProjectCard({ data }: { data: Record<string, unknown> }) {
  ...
  const keyDecisions = getArr(data, 'key_decisions') as string[] || getArr(data, 'keyDecisions') as string[];

  return (
    <div className="space-y-3">
      {summary && (
        <div>
          <p className="text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">Summary</p>
          <p className="whitespace-pre-wrap text-[11px] leading-relaxed text-gray-600 dark:text-gray-400">{summary}</p>
        </div>
      )}
      {goals.length > 0 && (
        <div>
      ...
      {milestones.length > 0 && (
        <div>
          <p className="text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">Milestones</p>
          <div className="space-y-1">
            {milestones.map((m, i) => {
              if (typeof m === 'string') {
                return <div key={i} className="text-[11px] text-gray-600 dark:text-gray-400">{m}</div>;
              }
              const rm = isRecord(m) ? m : {};
              const title = getStr(rm, 'title') || getStr(rm, 'name') || 'Milestone';
              const status = getStr(rm, 'status') || 'pending';
              const badgeColor = status === 'done' || status === 'completed' ? 'bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-300'
              ...
              return (
                <div key={i} className="flex items-center justify-between rounded border px-2 py-1 dark:border-gray-700">
                  ...
                  <span className={`rounded px-1.5 py-0.5 text-[10px] font-medium ${badgeColor}`}>{status}</span>
                </div>
              );
            })}
          </div>
      ...
      {keyDecisions.length > 0 && (
        <div>
          <p className="text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">Key Decisions</p>
          <ul className="list-disc space-y-0.5 pl-4 text-[11px] text-gray-600 dark:text-gray-400">
            {keyDecisions.map((d, i) => <li key={i}>{typeof d === 'string' ? d : JSON.stringify(d)}</li>)}
          </ul>
        </div>
      )}
    </div>
...
function LongTermRenderer({ content }: { content: string }) {
  ...
  if (isRecord(parsed)) {
    ...
    const sections = ['Recent activity', 'Pending items', 'Key decisions', 'Goals', 'Milestones', 'Notes'];
    const matched = sections.filter((s) => getStr(parsed, s) || getArr(parsed, s).length > 0 || isRecord(parsed[s]));
    if (matched.length >= 2) {
      return (
        <div className="space-y-2">
          {matched.map((key) => {
            ...
            return (
              <div key={key}>
                <p className="text-xs font-medium text-gray-700 dark:text-gray-300 mb-0.5">{key}</p>
                {typeof val === 'string' ? (
                  <p className="whitespace-pre-wrap text-[11px] leading-relaxed text-gray-600 dark:text-gray-400">{val}</p>
                ) : Array.isArray(val) ? (
                  <ul className="list-disc space-y-0.5 pl-4 text-[11px] text-gray-600 dark:text-gray-400">
                    {val.map((v, i) => <li key={i}>{typeof v === 'string' ? v : JSON.stringify(v)}</li>)}
                  </ul>
                ) : (
                  <pre className="whitespace-pre-wrap font-mono text-[11px] text-gray-600 dark:text-gray-400">{JSON.stringify(val, null, 2)}</pre>
                )}
              </div>
            );
          })}
  ...
  while (i < lines.length) {
    const line = lines[i] ?? '';
    if (/^#{1,2}\s/.test(line)) {
      elements.push(<p key={i} className="mt-2 text-sm font-semibold text-gray-800 dark:text-gray-200">{line.replace(/^#+\s/, '')}</p>);
      ...
      elements.push(
        <ul key={i} className="list-disc space-y-0.5 pl-4 text-[11px] text-gray-600 dark:text-gray-400">
      ...
      elements.push(
        <ol key={i} className="list-decimal space-y-0.5 pl-4 text-[11px] text-gray-600 dark:text-gray-400">
          {items.map((it, idx) => <li key={idx}>{it}</li>)}
        </ol>
      );
      continue;
      ...
      elements.push(<p key={i} className="text-[11px] leading-relaxed text-gray-600 dark:text-gray-400">{line}</p>);
    } else if (line.trim() === '') {
    ...
    <div className="space-y-1">
      {Object.entries(data).map(([k, v]) => (
        <div key={k} className="flex items-start gap-2">
          <span className="flex-shrink-0 text-[11px] font-medium text-gray-700 dark:text-gray-300">{k}:</span>
          {typeof v === 'string' ? (
            <span className="whitespace-pre-wrap text-[11px] text-gray-600 dark:text-gray-400">{v}</span>
            ...
            <span className="whitespace-pre-wrap font-mono text-[11px] text-gray-600 dark:text-gray-400">{JSON.stringify(v, null, 2)}</span>
          )}
        </div>
      ))}
    </div>
...
function LayerContent({ layer, content }: { layer: string; content: string }) {
  ...
  if (!isRecord(parsed)) {
    ...
    return <p className="whitespace-pre-wrap text-[11px] leading-relaxed text-gray-600 dark:text-gray-400">{content}</p>;
  }
                    ...
                    {isExpanded ? (
                      ...
                      <span className="text-[11px] text-gray-600 dark:text-gray-400">
                        {(() => {
                          ...
                          if (isRecord(parsed)) {
                            const summary = getStr(parsed, 'summary') || getStr(parsed, 'name') || getStr(parsed, 'title');
                            if (summary) return summary;
                          }
                          return m.content.slice(0, 200) + (m.content.length > 200 ? '...' : '');
                        })()}
                      </span>
                    )}
                  </span>
