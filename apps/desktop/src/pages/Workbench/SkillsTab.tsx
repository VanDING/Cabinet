...
export function SkillsTab() {
  ...
  return (
    <div>
          ...
          <div className="space-y-3">
            ...
            <div className="flex gap-3">
              <select
                ...
                className="border-border bg-surface-primary text-content-primary rounded-sm border px-3 py-2 text-sm"
              >
            ...
            <Button size="sm" fullWidth onClick={handleCreate} disabled={!formData.name.trim()}>
      ...
      {skills.length === 0 ? (
        <p className="text-content-tertiary py-4 text-sm">No skills found.</p>
      ) : (
            ...
            <Card key={s.id} padding="sm" className="group flex items-center justify-between">
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <Tag
                    variant={
                      s.kind === 'tool' ? 'info' : s.kind === 'prompt' ? 'success' : 'purple'
                    }
                  >
                    {s.kind}
                  </Tag>
                ...
                <p className="text-content-tertiary mt-0.5 text-xs">{s.description}</p>
              </div>
              <div className="flex items-center gap-1">
                <button
                  onClick={() => handleEdit(s)}
                  className="text-content-tertiary hover:text-accent px-2 py-1 text-xs opacity-0 transition-opacity group-hover:opacity-100"
                >
                  Edit
                </button>
                <button
                  onClick={() => handleDelete(s.id)}
                  className="text-content-tertiary hover:text-intent-danger px-2 py-1 text-xs opacity-0 transition-opacity group-hover:opacity-100"
                >
                  Del
                </button>
                <span className="text-content-tertiary text-xs">v{s.version}</span>
              </div>
            </Card>
          ))}
        </div>
