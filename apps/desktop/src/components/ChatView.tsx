            ...
            <>
              ...
              {msg.structuredOutputs && msg.structuredOutputs.length > 0 && (
                <StructuredOutputBlock
                  outputs={msg.structuredOutputs}
                  onAdoptDecision={(id, option) => {
                    // TODO: persist decision via API
                  }}
                  onRequestMore={(id) => {
                    // TODO: prompt AI for more info
                  }}
                  onRejectDecision={(id) => {
                    // TODO: mark decision as rejected
                  }}
                  onApproveDeliverable={(id) => {
                    // TODO: approve deliverable via API
                  }}
                  onRequestChanges={(id) => {
                    // TODO: request changes
                  }}
                  onDiscussDeliverable={(id) => {
                    // TODO: open discussion context
                  }}
                  onExpandDeliverable={(id) => {
                    // TODO: open full text modal
                  }}
                  onConfirmTasks={(id, tasks) => {
                    // TODO: persist tasks via API
                  }}
                  onAdjustTasks={(id) => {
                    // TODO: prompt AI to adjust
                  }}
                  onAddTask={(id) => {
                    // TODO: focus input for adding task
                  }}
                  onToggleTask={(id, taskId) => {
                    // TODO: toggle task status
                  }}
                  onViewWorkflowResult={(id) => {
                    // TODO: open full workflow report
                  }}
                  onRerunWorkflow={(id) => {
                    // TODO: rerun workflow
                  }}
                  onFixWorkflowIssue={(id, alert) => {
                    // TODO: create fix task
                  }}
                />
              )}
...
interface StructuredOutputBlockProps {
  ...
  onAdoptDecision?: (id: string, option: string) => void;
  onRequestMore?: (id: string) => void;
  onRejectDecision?: (id: string) => void;
  onApproveDeliverable?: (id: string) => void;
  onRequestChanges?: (id: string) => void;
  onDiscussDeliverable?: (id: string) => void;
  onExpandDeliverable?: (id: string) => void;
  onConfirmTasks?: (id: string, tasks: import('@cabinet/types').TaskItem[]) => void;
  onAdjustTasks?: (id: string) => void;
...
function StructuredOutputBlock({
  ...
  onRequestChanges,
  onDiscussDeliverable,
  onExpandDeliverable,
  onConfirmTasks,
  onAdjustTasks,
  onAddTask,
  onToggleTask,
  onViewWorkflowResult,
  onRerunWorkflow,
  onFixWorkflowIssue,
}: StructuredOutputBlockProps) {
  return (
          ...
          case 'deliverable':
            return (
              <DeliverableCard
                ...
                onDiscuss={onDiscussDeliverable}
                onExpand={onExpandDeliverable}
              />
            );
          case 'status_report':
            return <StatusReportCard key={output.id} output={output} />;
          case 'task_list':
            return (
              <TaskCard
                key={output.id}
          ...
          case 'workflow_result':
            return (
              <WorkflowResultCard
                key={output.id}
                output={output}
                onViewReport={onViewWorkflowResult}
                onRerun={onRerunWorkflow}
                onFixIssue={onFixWorkflowIssue}
              />
            );
          default:
            return null;
