...
export const MeetingList = memo(function MeetingList({ projectId }: Props) {
  ...
  return (
    <div className="flex h-full flex-col overflow-y-auto rounded-lg border bg-white">
      {loading ? (
        ...
        <>
          <div className="flex items-center justify-between border-b border-gray-200 px-3 py-2">
            <h3 className="text-xs font-semibold text-gray-600">Meetings</h3>
            <span className="text-xs text-gray-400">{meetings.length}</span>
          </div>
          {meetings.map((m) => (
            <div
              key={m.id}
              className="border-b border-gray-100 px-3 py-2 transition-colors hover:bg-gray-50:bg-gray-800/50"
            >
