import type { Theme } from './types';

export const warm: Theme = {
  id: 'warm',
  name: 'Warm',
  colors: {
    surface: {
      primary: '#FAF6F0',
      elevated: '#F0EBE3',
      surface2: '#F4EFE7',
      surface3: '#EDE8DF',
      overlay: '#FAF6F0',
      input: '#FFFFFF',
      muted: '#E8E2D8',
      sidebar: '#FAF6F0',
      floating: '#FAF6F0',
    },
    content: {
      primary: '#3D3929',
      secondary: '#5A5045',
      tertiary: '#A09880',
      inverse: '#FAF6F0',
      link: '#C0653A',
    },
    border: {
      color: '#E5D8C8',
      subtle: '#EDE2D4',
    },
    accent: {
      base: '#CC785C',
      hover: '#A9583E',
      muted: 'rgba(204, 120, 92, 0.08)',
      foreground: '#FFFFFF',
    },
    intent: {
      success: { color: '#7C8C6C', muted: 'rgba(124, 140, 108, 0.08)', foreground: '#FFFFFF' },
      danger: { color: '#C87878', muted: 'rgba(200, 120, 120, 0.08)', foreground: '#FFFFFF' },
      warning: { color: '#D4A85C', muted: 'rgba(212, 168, 92, 0.08)', foreground: '#3D3929' },
      info: { color: '#7C9AAC', muted: 'rgba(124, 154, 172, 0.08)', foreground: '#FFFFFF' },
      purple: { color: '#9B8EC4', muted: 'rgba(155, 142, 196, 0.08)', foreground: '#FFFFFF' },
    },
    code: {
      blockBg: '#F0EBE3',
      blockBorder: 'rgba(0, 0, 0, 0.06)',
      blockColor: '#3D3929',
      inlineBg: '#E8E2D8',
      inlineColor: '#C87878',
    },
    syntax: {
      keyword: '#C87878',
      string: '#7C8C6C',
      number: '#9B8EC4',
      comment: '#A09880',
      function: '#7C9AAC',
      variable: '#3D3929',
      tag: '#7C8C6C',
    },
    graph: {
      entityPerson: '#7C9AAC',
      entityProject: '#9B8EC4',
      entityConcept: '#7C8C6C',
      entityTechnology: '#D4A85C',
      entityDecision: '#C87878',
      entityMemory: '#A09880',
      edgeActive: '#7C9AAC',
      edgeInactive: '#A09880',
      nodeLabel: '#3D3929',
      bgGrid: '#E8E2D8',
      minimapMask: 'rgba(61,57,41,0.5)',
    },
    chart: {
      c1: '#7C9AAC',
      c2: '#7C8C6C',
      c3: '#D4A85C',
      c4: '#9B8EC4',
      c5: '#C87878',
      c6: '#CC785C',
      c7: '#6B6259',
      c8: '#3D3929',
    },
    brand: {
      accent: '#F6EAE3',
      success: '#F0F4EE',
      danger: '#F4E8E8',
      warning: '#F6F0E6',
      info: '#EBF0F4',
      purple: '#F0ECF4',
    },
    icon: { primary: '#3D3929', secondary: '#5A5045', accent: '#CC785C' },
  },
  style: {
    radius: { sm: '4px', md: '8px', lg: '12px', xl: '16px' },
    shadow: {
      sm: '0 1px 1px rgba(0, 0, 0, 0.02)',
      md: '0 1px 2px rgba(0, 0, 0, 0.03)',
      lg: '0 2px 8px rgba(0, 0, 0, 0.04)',
      popover: '0 4px 16px rgba(180,120,60,0.12), 0 1px 2px rgba(180,120,60,0.08)',
      modal: '0 8px 32px rgba(180,120,60,0.18), 0 2px 8px rgba(180,120,60,0.12)',
    },
    font: {
      family: "-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif",
      display: "Georgia, 'Times New Roman', serif",
      letterSpacing: '0',
      lineHeight: '1.6',
      sizeBase: '1rem',
      sizeSm: '0.875rem',
      sizeXs: '0.75rem',
      sizeLg: '1.125rem',
      sizeXl: '1.25rem',
      size2xl: '1.5rem',
      weightNormal: '400',
      weightMedium: '500',
      weightBold: '700',
    },
    border: { width: '1px', style: 'solid' },
    transition: { duration: '200ms', fast: '100ms', slow: '400ms', easing: 'ease-out' },
    opacity: { hover: '0.9', disabled: '0.45', overlay: '0.35' },
    glass: { blur: '0px', opacity: '1' },
    focusRing: { width: '2px', color: '#CC785C', offset: '2px' },
    selection: { bg: 'rgba(204, 120, 92, 0.2)', fg: '#3D3929' },
    scrollbar: { width: '6px', thumb: 'rgba(0, 0, 0, 0.12)', track: 'transparent' },
    bodyBg: '#F0EBE3',
  },
};
