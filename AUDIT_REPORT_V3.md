...
| Workflow StateGraph 编译 + fallback 双路径 | engine.ts 仍先尝试 StateGraph → 失败回退邻接图遍历。状态图编译仍偶尔失败                    |
| ManagerExecutor 合并                       | 不再公共导出但代码仍独立存在于 `manager-executor.ts` (233 行)                               |
...
| v2 审计建议                      | 原因分析                                                                                                            |
| -------------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| MCP 安全沙盒 T0-T3 集成          | MCP manager 有两层分类 (MCPSideEffectRisk + SafetyGuard) 但 T0-T3 信任级别检查在外部 SafetyGuard — 存在架构绕过路径 |
| ContextSlot 升级为 SharedDataBus | Agent Blackboard 作为更强替代被构建，但两者独立并行无同步桥接，形成概念重复                                         |
...
**⚠️ 剩余问题**: `SelfConsistencyEngine` 仍是独立对象而非 Observer，打破统一闭环管道抽象。
...
**⚠️ 剩余问题**: MCP 安全沙盒仍为两层分类 (MCPSideEffectRisk + SafetyGuard) 而非 T0-T3 集成。MCP manager 自身不感知信任级别。
...
**⚠️ 剩余问题**: `@cabinet/graph` 包 (753 行) 仍被 agent (2 文件) 和 workflow (1 文件) 引用，但 agent-loop.ts 不再使用。存在部分死代码风险。
...
| v2 问题                           | 当前状态                                                            |
| --------------------------------- | ------------------------------------------------------------------- |
| `_execute()` ~320 行闭包大        | 未变 — 仍 323 行，闭包量大，难以单元测试隔离                        |
| `conversationHistory` 语义模糊    | 未变 — 同时作为会话转录和跨循环缓冲区                               |
| `@cabinet/graph` 可能死代码       | ⚠️ agent-loop 不 import 它；但 agent-node + trace + workflow 仍使用 |
| SelfConsistencyEngine 非 Observer | 未变 — 打破统一管道抽象                                             |
| run/runStreaming 重复样板         | 未变 — ~40 行重复的错误处理/结果返回                                |
...
| StateGraph 编译 + fallback 双路径                         | 未变 — 仍先尝试 StateGraph → 失败回退邻接图遍历                                              |
...
| ManagerExecutor 独立文件                                  | ⚠️ 仍为独立文件 (233 行)，内部使用但不公共导出                                               |
...
CABINET.md 规定单文件不超过 500 行。当前有 35 个文件超过此上限：

| 文件                                                   | 行数  |
| ------------------------------------------------------ | ----- |
| apps/server/src/context.ts                             | 1,835 |
| apps/server/src/capabilities.ts                        | 1,330 |
| packages/agent/src/tools/index.ts                      | 1,301 |
| packages/agent/src/agent-loop.ts                       | 1,287 |
| apps/server/src/routes/workflows.ts                    | 1,277 |
| apps/server/src/routes/secretary/tool-dependencies.ts  | 1,263 |
| apps/server/src/secretary/agent-factory.ts             | 1,067 |
| packages/agent/src/daemon/agent-daemon.ts              | 780   |
| apps/server/src/routes/secretary/index.ts              | 775   |
| apps/server/src/routes/memory.ts                       | 768   |
| packages/memory/src/long-term.ts                       | 709   |
| apps/server/src/routes/agents.ts                       | 653   |
| packages/agent/src/**tests**/characterization.test.ts  | 640   |
| apps/server/src/routes/dashboard.ts                    | 639   |
| packages/agent/src/safety.ts                           | 643   |
| apps/server/src/secretary/skill-middleware.ts          | 635   |
| apps/desktop/src/pages/ProjectWorkplace.tsx            | 597   |
| packages/agent/src/adapters/harness/a2a.ts             | 584   |
| apps/desktop/src/components/layout/AppLayout.tsx       | 545   |
| apps/server/src/routes/secretary/agent-loop-factory.ts | 541   |
| packages/agent/src/**tests**/tools.test.ts             | 530   |
| packages/secretary/src/secretary-agent.ts              | 506   |

**建议**: 前 6 个文件 (均 >1,000 行) 应作为 P1 拆分目标。
...
packages/ (14):
  Layer 4: ui
  Layer 3: cli, decision, secretary, workflow, harness
  Layer 2: agent, gateway, memory, agent-sdk (private)
  Layer 1: types, events, storage, graph

apps/ (2):
  desktop (Tauri 2.0 + React 19)
  server (Hono + WebSocket)

总代码量: ~84,000 行 TypeScript/TSX (352 文件)
```
