...
| **UI 组件**  | Radix UI + Tailwind CSS 4.3    | 可定制、无障碍、暗色模式友好                                                 |
| **工作流画布** | `@xyflow/react` + `dagre`    | 可视化节点编辑器与自动布局                                                   |
| **数据获取** | `@tanstack/react-query`        | 服务端状态管理与缓存                                                         |
...
│   │   ├── src/
│   │   │   ├── index.ts
│   │   │   ├── state-graph.ts       # StateGraph 构建器与编译执行器
│   │   │   ├── annotation.ts        # 状态模式注解构建器
│   │   │   ├── checkpoint-store.ts  # 检查点持久化（链表结构）
│   │   │   ├── validation.ts        # 编译时图验证（6 道检查）
│   │   │   └── events.ts            # 流事件类型定义
│   │   └── package.json
...
│   │   ├── src/
│   │   │   ├── index.ts
│   │   │   ├── agent-loop.ts        # StateGraph 驱动的 TAOR 核心循环
│   │   │   ├── agent-node.ts        # Agent 节点工厂与执行器
│   │   │   ├── agent-handoff.ts     # 跨 Agent 状态交接与上下文压缩
│   │   │   ├── prompt-assembler.ts  # 模块化提示组装器（模块 → 完整 system prompt）
│   │   │   ├── prompt-shared.ts     # 共享提示规则与工具段生成
│   │   │   ├── context-builder.ts   # 上下文构建器
│   │   │   ├── context-monitor.ts   # 上下文健康监控（Smart/Dumb Zone）
│   │   │   ├── checkpoint.ts        # 检查点保存与恢复
│   │   │   ├── tool-executor.ts     # 工具调用执行器
│   │   │   ├── tool-pruner.ts       # 动态工具裁剪（按意图相关性）
│   │   │   ├── retry.ts             # 重试策略（指数退避，按错误类型分类处理）
│   │   │   ├── safety.ts            # 运行时四级安全检查
│   │   │   ├── interactive-sub-agent.ts # 交互式子 Agent（多轮会话 + 中途输入）
│   │   │   ├── built-in-skills.ts   # 内置技能注册（工作流设计师、Agent 创建器等）
│   │   │   ├── skill-registry.ts    # Skill 运行时注册与发现
│   │   │   ├── skill-loader.ts      # SKILL.md 加载与热重载
│   │   │   ├── dispatcher.ts        # Agent 路由与调度器
│   │   │   ├── trace.ts             # Graph 执行轨迹日志与终端调试
│   │   │   └── agent-roles.ts       # Agent 角色定义与 LLM 路由
│   │   └── package.json
...
│   │   ├── src/
│   │   │   ├── index.ts
│   │   │   ├── protocol.ts          # 会议协议与辩论流程定义
│   │   │   └── synthesis.ts         # 多顾问意见综合与结论生成
│   │   └── package.json
...
│   │   ├── src/
│   │   │   ├── index.ts
│   │   │   ├── blueprint-deployer.ts
│   │   │   ├── blueprint-parser.ts
│   │   │   └── blueprint-validator.ts
│   │   └── package.json
│   ├── agent-sdk/                    # @cabinet/agent-sdk（外部Agent SDK）
│   │   ├── src/
│   │   │   ├── index.ts
│   │   │   ├── slot-client.ts       # Context Slot读写客户端
│   │   │   ├── a2a-helper.ts        # A2A协议助手
│   │   │   └── types.ts             # SDK类型导出
│   │   └── package.json
...
│   └── bench/                        # 性能基准测试
├── packages/cli/                     # @cabinet/cli（命令行工具）
│   └── src/
│       └── index.ts                  # CLI 入口
├── docs/
│   ├── site/                         # VitePress 文档站点
...
| 表名            | 字段                                                                                                                        | 说明               |
| :-------------- | :-------------------------------------------------------------------------------------------------------------------------- | :----------------- |
| `projects`      | id, name, description, status, root_path, created_at                                                                        | 项目容器           |
...
| `agent_roles`   | id, type, name, description, system_prompt, model_tier, temperature, allowed_tools, is_builtin                             | Agent角色定义      |
...
| `settings`      | key TEXT PRIMARY KEY, value TEXT                                                                                             | 系统设置键值存储   |
| `metrics`       | id, name, value, tags_json, timestamp                                                                                       | 可观测性指标       |
...
| `/api/factory/workflows`         | GET/POST        | 列出 / 创建工作流                        |
...
| `/api/daemon/tasks`              | GET/POST        | 列出 / 创建后台任务                      |
| `/api/daemon/status`             | GET             | 获取 Daemon 运行状态                     |
| `/api/autopilots`                | GET/POST        | 列出 / 创建自动触发器                    |
| `/api/squads`                    | GET/POST        | 列出 / 创建 Squad 团队                   |
| `/api/slot/:taskId/read`         | GET             | 读取外部Agent上下文槽                    |
| `/api/slot/:taskId/write`        | POST            | 写入外部Agent发现                        |
| `/api/external/decisions`        | POST            | 外部Agent创建决策                        |
| `/api/external/deliverables`     | POST            | 外部Agent提交交付物                      |
| `/api/telemetry/report`          | POST            | 上报 Agent 遥测数据                      |
...
- 新增 A2A 协议客户端和 Agent SDK（`@cabinet/agent-sdk`）
