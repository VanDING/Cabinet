...
| **向量存储** | LanceDB | 嵌入式向量数据库，无外部服务依赖，与 TypeScript 原生集成 |
...
替代方案：**Vercel AI SDK** 提供统一的 `generateText` / `streamText` 接口，已适配 Anthropic、OpenAI、Google 等主流提供商。自研轻量路由层（`ModelRouter`）在此基础上按角色分流模型，保持接口统一。此方案零外部运行时依赖，与 Tauri 桌面端打包兼容。
...
│   │   │   ├── long-term.ts         # 长期记忆（LanceDB 语义检索）
...
| `workflow/verification-gate.ts` | @cabinet/workflow | 工作流**节点执行后** | 节点输出是否满足下游节点的输入schema，类型校验 |
...
- Workflow 节点类型定义支持 9 种节点
- 编译无错误

**关键文件**：

*primitives.ts*：Organization, Project, Employee, Skill, Workflow, Memory 等核心实体的类型定义。Employee 需同时支持 AI 管道和 HumanNode 两种形态。

*decisions.ts*：Decision 实体、DecisionType（strategic/action/execution/anomaly/evolution）、DecisionLevel（L0-L3）、DecisionStatus 状态机。
...
*events.ts*：MessageEnvelope 格式、MessageType 枚举（16 种消息类型）、各类 payload 类型（DecisionRequest, TaskOrder, DeliberationProposal 等）。

*boundaries.ts*：硬限制常量——最大辩论轮次、单次发言 Token 上限、重试次数、超时时间、成本上限等。
...
| **ModelRouter** | 根据角色（deep_think / fast_execute / default）路由到不同模型 |
...
**协议**：
1. 主代理接收议题，确定参与顾问
2. **成本预估**：根据顾问数量和议题复杂度估算 Token 消耗，超过 $0.50 需 Captain 确认
3. 子代理并行推理（各领域独立分析）
4. 验证代理对比差异点，仅对差异进行二次采样
5. 产出：共识 + 差异 + 少数派报告 + 无法调和的分歧
...
| **SkillNode** | 调用技能执行 |
| **ConditionNode** | 条件分支 |
| **ParallelNode** | 并行执行，等待汇合 |
| **HumanNode** | 暂停执行，向决策室发送审批请求 |
| **VerificationGate** | 节点执行后 schema 校验和类型检查 |

**验收标准**：
- 线性工作流（A→B→C）正确执行
- 条件分支按预期选择路径
- 并行节点同时执行，全部完成后汇合
- HumanNode 暂停并发送决策请求，收到审批后继续
- 节点失败时支持重试和超时处理
...
| `/api/decisions` | GET | 获取待决策列表（L2+），可按项目/级别筛选 |
| `/api/decisions/:id` | GET | 获取决策详情（深度审阅） |
| `/api/decisions/:id/approve` | POST | 审批决策 |
| `/api/decisions/:id/reject` | POST | 驳回决策 |
| `/api/factory/workflows` | GET | 列出工作流 |
| `/api/factory/workflows/:id/run` | POST | 启动工作流 |
| `/api/dashboard/summary` | GET | 获取 Dashboard 摘要数据 |
| `/api/settings/budget` | GET/PUT | 查询/修改预算配置 |
| `/api/settings/api-keys` | GET/POST/DELETE | 管理 API Key（加密存储） |
...
| 会议协议 | 同步推理 + 交叉验证 + 成本预估 | 减少无意义对话轮次，降低 Token 消耗，会前透明化成本 |
...
Captain 输入 → [Cabinet 对话界面] → SecretaryAgent (意图解析)
                              ...
                              (轻量审批 / 深度审阅 / 成本透明)
                                    ...
                                    Captain 裁决
                                         │
                                    Factory 执行
                                    (WorkflowEngine → VerificationGate)
                                         │
                                    Harness 后评估
                                    (质量闸门 → 不达标重试 → 达标放行)
                                         │
...
Agent 执行流程:
  ...
  │ verification-gate    │     Schema 校验 + 类型检查
  └─────────────────────┘
```
...
**技术栈**：TypeScript 全栈 + Tauri 桌面端  
**变更摘要（v1.0 → v2.0）**：
- 移除 Python/LiteLLM 依赖，改用 Vercel AI SDK + 自研路由层（纯 TypeScript）
- 向量存储从 ChromaDB 改为 LanceDB（嵌入式，无外部依赖）
- 补全项目记忆层（第四层记忆），消除与文档的不一致
- 明确 Harness / Agent / Workflow 三层安全/验证职责边界
- 新增 L0-L3 决策分级的具体判定算法
- 新增会议成本预估与预算管控机制
- 新增 API Key 加密存储与认证方案
- 新增数据库备份与恢复策略
- 新增 Tauri 桌面应用自动更新机制
- 新增 LLM 调用重试策略（按错误类型分类）
- 新增 Skills 完整生命周期管理
- 新增可观测性方案（结构化日志 + 指标）
- 新增三层安全/质量防护示意架构图
