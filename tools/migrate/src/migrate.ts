...
function migrate(sourcePath: string, targetPath: string): MigrationReport {
  ...
  try {
      ...
      for (const org of orgs) {
        target
          ...
          .run(
            org.id ?? org.uuid,
            org.name,
            org.captain_id ?? 'default',
            org.created_at ?? new Date().toISOString(),
          );
        report.organizations++;
      }
    } catch (e) {
      report.errors.push(`organizations: ${(e as Error).message}`);
    }
    ...
    try {
      ...
      report.errors.push(`decisions: ${(e as Error).message}`);
    }
    ...
    try {
      ...
      for (const ev of events) {
        target
          ...
          .run(
            ev.message_id ?? ev.id,
            ev.correlation_id ?? 'migrated',
            ev.causation_id ?? null,
            ev.type ?? ev.message_type ?? 'system_notification',
            ev.payload ?? '{}',
      ...
      report.errors.push(`events: ${(e as Error).message}`);
    }
    ...
    try {
      ...
      for (const emp of employees) {
        target
          ...
          .run(
            emp.id ?? emp.uuid,
            emp.project_id ?? 'default',
            emp.name,
            emp.role ?? 'advisor',
            emp.kind ?? 'ai',
            emp.pipeline_config ?? null,
            emp.persona ?? null,
      ...
      /* employees table may not exist in old version */
    }
