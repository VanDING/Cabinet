...
import type { A2AAgentCard, A2ATaskRequest, A2ATaskResponse, A2ATaskStatus } from './types.js';
...
export class A2AHarnessRuntime implements HarnessRuntime {
  ...
  async healthCheck(): Promise<boolean> {
    if (this.config.healthCheckUrl) {
      try {
        const resp = await fetchWithTimeout(this.config.healthCheckUrl, { method: 'GET' }, 5_000);
        return resp.ok;
      } catch {
        return false;
      }
  ...
  connectWebSocket(cabinetWsUrl: string): void {
    try {
      ...
      ws.onmessage = (msg: any) => {
        try {
          ...
          if (data.type === 'task_assigned' && data.task) {
            this.logger?.info(`A2A agent ${this.agentId} received task via WS`, {
      ...
      this.logger?.warn(`A2A agent ${this.agentId} WebSocket connection failed`, {
        error: String(err),
      });
      this.ws = null;
    }
  ...
  // Reuse module-level fetch helper
