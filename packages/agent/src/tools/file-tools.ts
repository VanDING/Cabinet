...
export interface FileToolDeps {
  ...
  writeFile: (path: string, content: string, overwrite?: boolean) => Promise<{ written: boolean; skipped: boolean }>;
  editFile: (path: string, oldString: string, newString: string, replaceAll?: boolean) => Promise<{ changed: boolean; occurrences: number }>;
  applyPatch: (path: string, diff: string) => Promise<{ applied: boolean; hunksApplied: number; hunksFailed: number }>;
  ...
  indexProject: (projectId: string, rootPath: string, force?: boolean) => Promise<{ indexed: number; skipped: number; errors: number }>;
}
...
export function createFileTools(deps: FileToolDeps): ToolDefinition[] {
  return [
    ...
    {
      name: 'write_file',
      description: 'Create a new file or overwrite an existing one. Set overwrite: false to prevent accidental overwrites.',
      execute: async (args: Record<string, unknown>) => {
        const filePath = args.path as string;
        ...
        try {
          ...
          if (result.skipped) {
            return { skipped: true, path: filePath, reason: 'File exists and overwrite is disabled' };
          }
    ...
    {
      name: 'edit_file',
      description: 'Replace old_string with new_string in a file. Use replace_all: true to replace all occurrences.',
      execute: async (args: Record<string, unknown>) => {
        const filePath = args.path as string;
    ...
    {
      name: 'apply_patch',
      description: 'Apply a unified diff patch to a file. The diff header (--- a/path, +++ b/path) identifies the target file.',
      execute: async (args: Record<string, unknown>) => {
        const filePath = args.path as string;
        ...
        try {
          ...
          return { applied: result.applied, path: filePath, hunksApplied: result.hunksApplied, hunksFailed: result.hunksFailed };
        } catch (e) {
          return { error: (e as Error).message };
        }
      },
    ...
    {
      name: 'index_project',
      description: 'Scan and index all source files in a project directory for semantic search. Call this after attaching a project for the first time, or use force: true to re-index.',
      execute: async (args: Record<string, unknown>) => {
