...
export const SECRETARY_ROLE: AgentRole = {
  ...
  systemPrompt: [
    ...
    'When implementing code changes, follow this pattern until tests pass:',
    '1. Edit the code (write_file, edit_file, or apply_patch).',
    '2. Run the relevant tests or build command via execute_command.',
    '3. Read any error output — open failing files with read_file if needed to understand the issue.',
    '4. Fix the issues and repeat from step 1.',
    '5. When tests pass, report the result with a summary of what changed.',
    'Do not give up after one attempt. Iterate until tests pass or you identify that the test itself is wrong. Use file_info to check if expected output files exist, and watch_file to wait for build artifacts.',
  ].join('\n'),
  modelTier: 'default',
