...
import type { WorkflowRun, WorkflowHandlers, WorkflowEdge, AgentLoopHandle } from './engine.js';
import {
  buildAdjacencyGraph,
  buildNodeInput,
  resolveVariable,
...
export class NodeExecutor {
  ...
  async runNode(
    ...
    switch (node.type) {
      ...
      case 'end':
        ...
        break;
      case 'agentGroup':
        output = await this.execAgentGroup(node, run, nodeMap, executeNode);
        break;
      case 'llm':
        output = await this.execLlm(node, run, previousOutputs);
        break;
      case 'skill':
        output = await this.execSkill(node, previousOutputs);
        break;
      case 'tool':
        output = await this.execTool(node, run);
        break;
      case 'code':
        output = await this.execCode(node, run);
        break;
      case 'workflow':
        output = await this.execWorkflow(node, previousOutputs);
        break;
      case 'ifElse':
        output = await this.execIfElse(node, run, previousOutputs);
        break;
      case 'loop':
        output = await this.execLoop(node, run, nodeMap, executeNode);
        break;
      case 'parallel':
        output = await this.execParallel(node, run, nodeMap, executeNode);
        break;
      case 'knowledgeBase':
        output = await this.execKnowledgeBase(node, previousOutputs);
        break;
      case 'approval':
        output = await this.execApproval(node, run);
        break;
      case 'human':
        output = await this.execHuman(node, run);
        break;
      case 'externalAgent':
        output = await this.execExternalAgent(node, run, previousOutputs);
        break;
      case 'manager':
    ...
    if (node.output?.schema || node.output?.role) {
      step.contract = {
        schema: node.output?.schema as
          | Record<string, 'string' | 'number' | 'boolean' | 'json' | 'file'>
          | undefined,
  ...
  private async execAgentGroup(
    ...
    if (!this.deps.handlers.createAgentLoop) {
      return 'No agent handler registered';
    }
    ...
    const childEdges = this.deps.currentEdges.filter(
  ...
  private async execLlm(
    ...
    if (run._agentLoop) {
      ...
      return await withTimeout(
        this.deps.handlers.aiAgent(node, previousOutputs),
        timeoutMs,
        `LLM ${node.id}`,
      );
    }
  ...
  private async execWorkflow(node: WorkflowNodeDef, previousOutputs: string): Promise<string> {
    ...
    if (node.synchronous === false) {
      this.deps.handlers.runSubWorkflow(node.workflowId, subInput).catch((err) => {
        console.error(
          `[WorkflowEngine] Fire-and-forget sub-workflow ${node.workflowId} failed:`,
  ...
  private async execLoop(
    ...
    for (let i = 0; i < maxIter; i++) {
      ...
      if (node.loopType === 'condition' && node.loopCondition) {
        const condResult = await this.evaluateCondition(
          node.loopCondition,
          run.steps.map((s) => s.output).join('\n'),
          run,
        );
        if (!condResult) break;
      }
  ...
  private async execParallel(
    ...
    const promises = childNodes.map(async (child) => {
      ...
      return childStep
        ? { nodeId: child.id, output: childStep.output }
        : { nodeId: child.id, output: '' };
    });
  ...
  private async execExternalAgent(
    node: WorkflowNodeDef,
    run: WorkflowRun,
    previousOutputs: string,
  ): Promise<string> {
  ...
  private async execManager(
    ...
    const managerDeps: ManagerContextDeps = {
      ...
      synthesizeWithLLM: async (prompt) => {
        if (!this.deps.handlers.aiAgent)
          throw new Error('No aiAgent handler for manager synthesis');
        return this.deps.handlers.aiAgent(node, prompt);
      },
    ...
    const nodeInput = buildNodeInput(run, node.id, this.deps.currentEdges);
    return await runManagerNode(node, nodeInput, managerDeps);
  }
