/**
 * Static blueprint validator for OrganizeAgent outputs.
 *
 * Checks structural correctness before deployment:
 *   - Agent references exist in the registry
 *   - Workflow step dependencies are valid
 *   - Condition branches point to real steps
 *   - Authorization rules cover all decision points
 *   - Quality gates map to actual steps
...
/** Validate an OrganizeAgent blueprint for structural correctness. */
export function validateBlueprint(
  ...
  // Collect new agents that will be created
  ...
  // 2. Check workflow step input references
  ...
  // 3. Check condition branch targets
  for (const step of steps) {
  ...
  // 5. Check authorization coverage for humanApproval nodes
  ...
  // 6. Check quality gates correspond to actual steps
...
/** Detect circular dependencies in a step graph using DFS. */
function detectCircularDependencies(steps: BlueprintWorkflowStep[]): string[][] {
  const stepIds = new Set(steps.map((s) => s.id));
