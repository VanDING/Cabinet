...
async def test_publish_with_no_subscribers_does_not_error():
    ...
    envelope = MessageEnvelope(
        sender="test", recipients=[],
        message_type="test.unhandled", payload={"foo": "bar"},
    )
    await bus.publish(envelope)
    stored = bus._store.get_by_type("test.unhandled")
    assert len(stored) == 1
    assert stored[0].message_type == "test.unhandled"
