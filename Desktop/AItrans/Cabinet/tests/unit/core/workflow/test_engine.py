...
@pytest.mark.asyncio
async def test_empty_workflow_raises_value_error():
    ...
    wf = Workflow(
        project_id=uuid4(), name="empty", kind="composite_skill",
        nodes=[], edges=[],
    )
    engine = WorkflowEngine(agent_factory=AsyncMock())
    with pytest.raises(ValueError, match="no trigger node"):
        await engine.run(wf, {})


@pytest.mark.asyncio
async def test_loop_zero_iterations():
    from cabinet.models.workflows import (
        Workflow, LoopNode, EndNode, TriggerNode, WorkflowEdge,
    )
    from cabinet.core.workflow.engine import WorkflowEngine
    from unittest.mock import AsyncMock
    from uuid import uuid4

    trigger = TriggerNode(id=uuid4(), trigger_type="manual")
    loop_body = EndNode(id=uuid4())
    loop = LoopNode(
        id=uuid4(), loop_type="count", max_iterations=0,
        body_entry_id=loop_body.id,
    )
    end = EndNode(id=uuid4())
    wf = Workflow(
        project_id=uuid4(), name="zero_loop", kind="composite_skill",
        nodes=[trigger, loop, loop_body, end],
        edges=[
            WorkflowEdge(source_node_id=trigger.id, target_node_id=loop.id),
            WorkflowEdge(source_node_id=loop.id, target_node_id=end.id),
        ],
    )
    engine = WorkflowEngine(agent_factory=AsyncMock())
    result = await engine.run(wf, {})
    assert result.get("__end__", {}).get("status") == "completed"
