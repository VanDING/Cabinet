...
Cabinet 提供两层 Agent 编排：

### 主路径：Room 编排

所有用户可见的任务通过 Six-Room 模型流转，由 `RoomEventWiring` 和 EventBus 驱动：

```
Captain → Secretary → Meeting → Strategy → Decision → Office → Summary
```

每个 Room Service 通过 `AgentFactory.create_agent()` 按需创建 Agent 实例，Agent 的生命周期受 Room 事件控制。

### 辅助路径：Agent 自主协作

Agent 在执行过程中需要同其他 Agent 协作时（例如一个 Agent 发现需要另一个 Agent 的专业知识），
通过 `MailboxRouter` 发送定向消息，通过 `HandoffManager` 请求任务移交。

- **AgentPool**：Agent 实例的缓存层，避免每次请求都创建新的 LLM Agent
- **MailboxRouter**：Agent 间消息路由，支持 request/response/notify/broadcast
- **HandoffManager**：Agent 间任务移交，支持 accept/reject/超时

### 约束

1. Handoff 仅限 Agent 间自主触发，不绕过 Room 调度
2. 所有对外的用户操作必须通过 Room 模型
3. AgentPool 作为性能优化层，不影响 Room 的编排语义
