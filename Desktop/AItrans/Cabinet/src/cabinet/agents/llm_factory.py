...
class LLMAgentFactory:
    ...
    async def create_agent(self, agent_id: UUID, role: str) -> LiteLLMAgent:
        ...
        if self._pool is not None:
            try:
                pooled = await self._pool.acquire(role=role, timeout=5.0)
                prompt = pooled.employee.personality or self._role_prompts.get(role, "")
                agent = LiteLLMAgent(
                    pooled.employee, self._gateway,
                    system_prompt=prompt, memory_store=self._memory_store,
                )
                agent._pool_entry = pooled  # Tag for release
                return agent
            except Exception:
