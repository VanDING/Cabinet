        ...
        if encryption_key is not None:
            ...
            if len(derived) != 44:
                ...
                if os.path.exists(salt_file):
                    ...
                    salt = None
                derived = KeyVault._derive_key(derived, salt=salt)
            # else: 44-char = Fernet key, use as-is
        else:
            ...
            if key_file:
                ...
                # No salt file needed -- we generated a Fernet key directly
    ...
    def _derive_key(material: bytes, salt: bytes | None = None) -> bytes:
        if len(material) == 44:
            try:
                ...
                return material  # Already a valid Fernet key
            except Exception:
                pass
        if salt is None:
            salt = os.urandom(16)
