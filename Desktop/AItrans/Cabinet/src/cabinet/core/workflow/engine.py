...
class WorkflowEngine:
    ...
    async def run(
        ...
        try:
            ...
            try:
                from cabinet.core.observability import WORKFLOW_EXECUTION
                duration = _time.monotonic() - wf_start
                WORKFLOW_EXECUTION.labels(kind=workflow.kind).observe(duration)
            except ImportError:
                pass
